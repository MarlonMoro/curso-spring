...
public class ResourceExceptionHandler {
	...
	public ResponseEntity<StandardError> validation(MethodArgumentNotValidException e, HttpServletRequest request) {
		...
		ValidationError err = new ValidationError(HttpStatus.BAD_REQUEST.value(), "Erro de validação",
				System.currentTimeMillis());
		// For para percorrer a lista de erros (que está em
		// "MethodArgumentNotValidException e"
		// Para cada erro X resultante do e.getBindResult().getFieldErros
		for (FieldError x : e.getBindingResult().getFieldErrors()) {
			err.getErrors(x.getField(), x.getDefaultMessage());
		}
	...
	public ResponseEntity<StandardError> amazomService(AmazonServiceException e, HttpServletRequest request) {
